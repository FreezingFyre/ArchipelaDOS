...
| | |
|-|-|
| `!hint item <item> [slot: <slot>]`         | Use a hint for the given `item` (or group); optionally filtered by `slot` |
| `!hint location <location> [slot: <slot>]` | Use a hint to see what is at the given `location` (or group); optionally filtered by `slot` |
